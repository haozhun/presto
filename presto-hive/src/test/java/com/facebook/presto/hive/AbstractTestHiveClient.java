...
public abstract class AbstractTestHiveClient
{
            ...
            throws Exception
    {
        try (Transaction transaction = newTransaction()) {
            ...
            ConnectorSplitSource splitSource = splitManager.getSplits(transaction.getTransactionHandle(), session, getOnlyElement(tableLayoutResults).getTableLayout().getHandle(), ConnectorSplitManager.SplitSchedulingStrategy.ALL_AT_ONCE);
    ...
    {
        try (Transaction transaction = newTransaction()) {
            splitManager.getSplits(transaction.getTransactionHandle(), newSession(), invalidTableLayoutHandle, ConnectorSplitManager.SplitSchedulingStrategy.ALL_AT_ONCE);
        }
    }
    ...
    {
        try (Transaction transaction = newTransaction()) {
            ConnectorSplitSource splitSource = splitManager.getSplits(transaction.getTransactionHandle(), newSession(), emptyTableLayoutHandle, ConnectorSplitManager.SplitSchedulingStrategy.ALL_AT_ONCE);
            // fetch full list
            ...
            throws Exception
    {
        try (Transaction transaction = newTransaction()) {
            ...
            try {
                getSplitCount(splitManager.getSplits(transaction.getTransactionHandle(), session, getOnlyElement(tableLayoutResults).getTableLayout().getHandle(), ConnectorSplitManager.SplitSchedulingStrategy.ALL_AT_ONCE));
            ...
            throws Exception
    {
        try (Transaction transaction = newTransaction()) {
            ...
            ConnectorSplitSource splitSource = splitManager.getSplits(transaction.getTransactionHandle(), session, layoutHandle, ConnectorSplitManager.SplitSchedulingStrategy.ALL_AT_ONCE);
            ...
            throws Exception
    {
        List<ConnectorTableLayoutResult> tableLayoutResults = transaction.getMetadata().getTableLayouts(
                ...
                Optional.empty());
        ConnectorTableLayoutHandle layoutHandle = getOnlyElement(tableLayoutResults).getTableLayout().getHandle();
        List<ConnectorSplit> splits = getAllSplits(splitManager.getSplits(transaction.getTransactionHandle(), session, layoutHandle, ConnectorSplitManager.SplitSchedulingStrategy.ALL_AT_ONCE));
            ...
            throws InterruptedException
    {
        try (Transaction transaction = newTransaction()) {
            ...
            List<ConnectorTableLayoutResult> tableLayoutResults = metadata.getTableLayouts(session, tableHandle, new Constraint<>(tupleDomain, bindings -> true), Optional.empty());
            ConnectorTableLayoutHandle layoutHandle = getOnlyElement(tableLayoutResults).getTableLayout().getHandle();
            return getAllSplits(splitManager.getSplits(transaction.getTransactionHandle(), session, layoutHandle, ConnectorSplitManager.SplitSchedulingStrategy.ALL_AT_ONCE));
        }
    }
