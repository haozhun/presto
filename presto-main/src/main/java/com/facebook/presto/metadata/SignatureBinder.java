...
public class SignatureBinder
{
    ...
    public Optional<BoundVariables> bindVariables(List<? extends Type> actualArgumentTypes)
    {
        ...
        if (!appendArgumentConstraintsTo(constraintSolvers, actualArgumentTypes)) {
            return Optional.empty();
        }

        return solve(constraintSolvers.build());
    }

    public Optional<BoundVariables> bindVariables(List<? extends Type> actualArgumentTypes, Type actualReturnType)
    {
        ImmutableList.Builder<TypeConstraintSolver> constraintSolvers = ImmutableList.builder();
        if (!appendConstraintSolverTo(constraintSolvers, declaredSignature.getReturnType(), actualReturnType, false)) {
            return Optional.empty();
        }
        if (!appendArgumentConstraintsTo(constraintSolvers, actualArgumentTypes)) {
            return Optional.empty();
     ...
     * <li>p used in combination with different literal variables: decimal(p,s1) and decimal(p,s2)
     * </ul>
     */
    ...
    private boolean appendArgumentConstraintsTo(ImmutableList.Builder<TypeConstraintSolver> resultBuilder, List<? extends Type> actualTypes)
    {
        ...
        for (int i = 0; i < formalTypeSignatures.size(); i++) {
            ...
            if (!appendConstraintSolverTo(resultBuilder, formalTypeSignature, actualType, allowCoercion)) {
                return false;
            }
        }
        return true;
    ...
    private boolean appendConstraintSolverTo(ImmutableList.Builder<TypeConstraintSolver> resultBuilder, TypeSignature formalTypeSignature, Type actualType, boolean allowCoercion)
    {
        Set<String> typeVariables = typeVariablesOf(formalTypeSignature);
        Set<String> longVariables = longVariablesOf(formalTypeSignature);
        resultBuilder.add(new TypeRelationshipConstraintSolver(formalTypeSignature, typeVariables, longVariables, actualType, allowCoercion));
        if (typeVariables.isEmpty() && longVariables.isEmpty()) {
            return true;
        }
        ...
        if (actualTypeTypeParameters.size() != formalTypeSignatureParameters.size()) {
            return false;
        }

        for (int i = 0; i < formalTypeSignatureParameters.size(); i++) {
            TypeSignatureParameter formalTypeParameter = formalTypeSignatureParameters.get(i);
            ...
            if (!appendConstraintSolverTo(resultBuilder, typeSignature.get(), actualTypeTypeParameters.get(i), allowCoercion && isCovariantTypeBase(formalTypeSignature.getBase()))) {
                return false;
            }
        }
    ...
    private Optional<BoundVariables> solve(List<TypeConstraintSolver> constraints)
    {
        ...
        for (TypeConstraintSolver constraint : constraints) {
            if (!constraint.isSatisfied(boundVariablesBuilder)) {
                return Optional.empty();
            }
        }
    ...
    private interface TypeConstraintSolver
    {
        ...
        boolean isSatisfied(BoundVariables.Builder bindings);
    }
            ...
            implements TypeConstraintSolver
    {
        ...
        @Override
        public boolean isSatisfied(BoundVariables.Builder bindings)
        {
            return true;
        }
            ...
            implements TypeConstraintSolver
    {
        ...
        public boolean isSatisfied(BoundVariables.Builder bindings)
        {
            ...
            if (allowCoercion) {
                return typeManager.canCoerce(actualType, typeManager.getType(boundSignature));
            }
            return actualType.getTypeSignature().equals(boundSignature);
        }
    }
}
