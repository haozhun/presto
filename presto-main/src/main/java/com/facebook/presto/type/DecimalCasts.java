...
public final class DecimalCasts
{
    ...
    public static final SqlScalarFunction DECIMAL_TO_VARCHAR_CAST = castFunctionFromDecimalTo(createUnboundedVarcharType().getTypeSignature(), "shortDecimalToVarchar", "longDecimalToVarchar");
    public static final SqlScalarFunction VARCHAR_TO_DECIMAL_CAST = castFunctionToDecimalFrom(parseTypeSignature("varchar(y)", ImmutableSet.of("y")), "varcharToShortDecimal", "varcharToLongDecimal");
    ...
    private static SqlScalarFunctionBuilder castFunctionToDecimalFromBuilder(TypeSignature from, String... methodNames)
    {
        Signature signature = Signature.builder()
                .kind(SCALAR)
                .operatorType(CAST)
                .typeVariableConstraints(withVariadicBound("X", StandardTypes.DECIMAL))
                .argumentTypes(from)
